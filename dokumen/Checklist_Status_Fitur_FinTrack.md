...
| 4 | Dashboard keuangan | P0 | Selesai | Dashboard menghitung saldo, income/expense bulan ini, cashflow, activity terbaru termasuk transfer, tagihan, saldo dompet berlogo/icon, dan insight. | Insight masih sederhana. | Tingkatkan setelah core transaksi/tagihan stabil. |
...
| 21 | Filter dan daftar transaksi | P0 | Selesai | Halaman transaksi memiliki filter kustom, merangkum saldo global, dan menampilkan riwayat transfer (melalui UNION). | Semua input filter/modal sudah memakai custom component. | Pertahankan. |
...
| 30 | Laporan keuangan | P1 | Selesai | Reports mendukung periode, filter kategori/type/dompet/search dengan FormDropdown/DatePicker/Autocomplete, summary, tren, kategori, dompet, status tagihan, tabel activity transfer, pagination, detail modal, dan CSV client-side. | PDF/Excel belum ada. | Tambah export server-side saat tahap lanjutan. |
...
| 32 | Integrasi Telegram link akun | P1 | Selesai | TelegramController membuat OTP, webhook menyimpan chat_id, halaman Telegram menampilkan status, kode aktif, instruksi kirim kode ke bot, dan log pesan. | Data integrasi belum di bot_integrations. | Pertimbangkan migrasi saat multi-platform distabilkan. |
...
| 2026-05-23 | Halaman Telegram dirapikan agar flow link akun fokus pada generate kode dan kirim kode ke bot; validasi OTP web yang membingungkan dihapus dari UI. |
...
| 2026-05-23 | Halaman transaksi diupdate agar merangkum Total Global dari hasil filter aktif secara akurat, dan tabel riwayat transaksi kini ikut menampilkan data Transfer antar dompet yang digabung menggunakan UNION (dengan integrasi komponen UI kustom pada filter dan form). |
| 2026-05-24 | Halaman laporan memakai FormDropdown, DatePicker, Autocomplete, filter dompet/search/per-page, tabel detail activity termasuk transfer, pagination, dan modal detail. |
| 2026-05-24 | Dashboard menampilkan transfer pada transaksi terakhir, saldo per dompet memakai logo provider/custom logo atau fallback icon, dan tooltip cashflow dinaikkan ke layer atas chart. |
